...
class MedGuardLogger {
    constructor(component, sessionId = null) {
        ...
        this.monitoringUrl = isBrowser 
            ? 'http://localhost:3005' 
            : (typeof process !== 'undefined' && process.env ? process.env.MONITORING_URL || 'http://localhost:3005' : 'http://localhost:3005');
        this.enabled = isBrowser 
            ? true 
            : (typeof process !== 'undefined' && process.env ? process.env.ENABLE_LOGGING !== 'false' : true);
        this.queue = [];
        this.isOnline = true;
